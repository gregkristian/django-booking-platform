...
class TestEmployeeRegistrationForm(TestCase):
    ...
    def test_employee_registration_form_valid(self):
        form = VisitorRegistrationForm(data=self.valid_user)
    ...
    def test_meta_data(self):
        self.assertEqual(VisitorRegistrationForm._meta.model, User)

        expected_fields = ["first_name", "last_name", "email", "password1", "password2", "gender"]
        for field in expected_fields:
            self.assertIn(field, VisitorRegistrationForm._meta.fields)
...
class TestEmployerRegistrationForm(TestCase):
    ...
    def test_field_required(self):
        form = OwnerRegistrationForm(data={})
    ...
    def test_employee_registration_form_valid(self):
        form = OwnerRegistrationForm(data=self.valid_user)
        self.assertEqual(True, form.is_valid(), "Invalid form")

    def test_invalid_email(self):
        data = self.valid_user
        data["email"] = "test"
        form = OwnerRegistrationForm(data=data)
        self.assertFalse(form.is_valid(), "Invalid email")
    ...
    def test_meta_data(self):
        self.assertEqual(OwnerRegistrationForm._meta.model, User)
